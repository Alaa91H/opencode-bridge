...
class TaskServiceV3:
    ...
    The database claim query already refuses a second running task for the same
    owner. Multiple workers therefore improve throughput without making one
    user's edits race each other.
    """
    ...
    async def _run(self, worker_id: int) -> None:
        while not self._stop.is_set():
            try:
                ...
                log.exception("فشل عامل المهام %s", worker_id)
